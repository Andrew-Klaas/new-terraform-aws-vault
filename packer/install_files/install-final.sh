...
function bootstrap_acl {
  ...
  MT=`curl --request PUT http://127.0.0.1:${CONSUL_PORT}/v1/acl/bootstrap |cut -d'"' -f4`
  echo $MT
}
...
function set_agent {
  ...
  local mt="$1"
  AT=`curl  --request PUT  --header "X-Consul-Token: ${mt}" --data '{"Name": "Agent Token", "Type": "client", "Rules": "node \"\" { policy = \"write\" } service \"\" { policy = \"read\" }"}' http://127.0.0.1:${CONSUL_PORT}/v1/acl/create | cut -d'"' -f4`
  echo $AT
}
...
function set_vault {
  ...
  local mt="$1"
  VT=`curl --request PUT  --header "X-Consul-Token: ${mt}" --data '{"Name": "Vault Token", "Type": "client", "Rules": "node \"\" { policy = \"write\" } service \"vault\" { policy = \"write\" } agent \"\" { policy = \"write\" }  key \"vault\" { policy = \"write\" } session \"\" { policy = \"write\" } "}' http://127.0.0.1:${CONSUL_PORT}/v1/acl/create | cut -d'"' -f4`
  echo ${VT}
}
